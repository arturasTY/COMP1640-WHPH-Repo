...
try {
        $result = $db_connection->query("SELECT * FROM Grp_Ideas LIMIT 20");
    }
...
//        exit();
   ...
   foreach ($result as $row) {
        $ideas[] = array('Id' => $row['IdeasID'], 'ideastext' => $row['IdeasText'], 'ideacomment' => $row['Comments'], 'emp' => $row['UserID'], 'files' => $row['FileUploads'], 'views' => $row['views']);
   }
        ...
        <div class="row">
          ...
          <main role="main" class="col-md-9 ml-sm-auto col-lg-10 pt-3 px-4">
            ...
            <div class="export-block">
              <h3 class="submit-id">All Submitted Ideas</h3>
            ...
            <div class="table-responsive">
                ...
                <tbody>
                 <?php foreach($ideas as $idea) : ?>
                  <tr>
                    ...
                    <td><?php echo substr($idea['ideastext'], 0, 30); ?></td>
                    <td><?php echo substr($idea['ideacomment'], 0, 30); ?></td>
                    <td>
                        <?php
                        
                        $idi = $idea['emp'];
                        try {
                              $result = $db_connection->query("SELECT Name AS nam FROM Grp_employee INNER JOIN Grp_Ideas ON Grp_Ideas.UserID = Grp_employee.empID WHERE Grp_Ideas.UserID = $idi");
                            $result ->execute(); 
                          }
                          catch(PDOException $e) {
              ...
              <div class="card w-100">
                <div class="card-body w-100">
                    <h4 class="submit-id">Download All User Uploaded Files as a ZIP Archive</h4>
                    <form action="download_zip.php" method="post" class="zip-form">
                        <input type="submit" name="download" class="btn cta" value="Export ZIP">
                    </form>
                </div>
              </div>
            </div>
