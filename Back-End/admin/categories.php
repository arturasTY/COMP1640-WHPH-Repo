...
//        exit();
   ...
   foreach ($result as $row) {
        $cats[] = array('Id' => $row['CategoryID'], 'cat_name' => $row['Categories']);
   }


try {
        $result = $db_connection->query("SELECT CategoryID FROM Grp_Categories");
    }
    catch(PDOException $e) {
//        $output = "Error occured when adding author, please try again" . $e->getMessage();
//        include "error.html.php";
//        exit();
 ...
 if(isset($_POST['cat'])) {
    ...
    try {
        $sql = "INSERT INTO Grp_Categories SET Categories = :name";
        $s = $db_connection->prepare($sql);
        $s->bindValue(':name', $_POST['cat_name']);
        $s->execute();
...
//        exit;
        ...
        header("location: categories.php"); //Redirecting back to jokes template
        exit;
}
...
if(isset($_POST['delete'])) {
       ...
       try {
        $sql = "DELETE FROM Grp_Categories WHERE CategoryID = :id";
        $s = $db_connection->prepare($sql);
        $s->bindValue(':id', $_POST['catid']);
        $s->execute();
        
    } catch (PDOException $e) {
//        $output = "Error occured when deleting category, please try again" . $e->getMessage();
//        include "error.html.php";
//        exit;
    }
        header("location: categories.php"); //Redirecting back to jokes template
...
<head>
    <meta charset="UTF-8">
    <title>WHPH Administrator | Categories</title>
        ...
        <div class="row">
          ...
          <main role="main" class="col-md-9 ml-sm-auto col-lg-10 pt-3 px-4">
            ...
            <div class="export-block">
              <h3 class="submit-id">All Categories</h3>
                  ...
                  <tr>
                    ...
                    <td><?php echo $cat['cat_name']; ?></td>
                      <td><?php 
                          ...
                          try {
                              $result = $db_connection->query("SELECT COUNT(Grp_Ideas.IdeasID) AS counter FROM Grp_Ideas INNER JOIN Grp_Categories ON Grp_Ideas.CategoriesID = Grp_Categories.CategoryID WHERE Grp_Ideas.CategoriesID = $catid ");
                            $result ->execute(); 
                          }
                          catch(PDOException $e) {
//        $output = "Error occured when adding author, please try again" . $e->getMessage();
...
//        exit();
                    ...
                    <td class="text-center">
                        <?php if(isset($counter) && $counter < 1) : ?>
                           ...
                           <button type="submit" class="btn btn-danger btn-sm w-100" name="delete">Delete</button>
                        </form>
            ...
            <h3 class="submit-id" style="margin-top:10px;">Add a new Category</h3>
            <form action="" method="post">
              <div class="form-group" style="display: flex;">
                  <input type="text" class="form-control" name="cat_name" id="exampleInputCat" aria-describedby="emailHelp" placeholder="Enter category name">
                  <button type="submit" name="cat" class="btn cta" style="flex-basis: 40%; margin-left: 10px;">Submit</button>
              </div>
            </form>
          </main>
        </div>
